...
type Platform = {
  name: string;
  color: string;
  // Builds the deep-link URL. If the platform supports prefilling, embed the prompt.
  url: (q: string) => string;
  // Whether the URL prefills the prompt (vs. relying on the clipboard paste).
  prefills: boolean;
...
const aiPlatforms: Platform[] = [
  ...
  },
  {
    name: "Gemini",
    color: "#4285F4",
    // Gemini has no reliable prompt query param — rely on clipboard paste.
  ...
  },
  {
    name: "Claude",
    color: "#D97757",
    // Claude removed its ?q= prefill param — open a new chat and rely on clipboard paste.
  ...
  },
  {
    ...
    logo: (
      <svg viewBox="0 0 24 24" className="w-5 h-5" fill="currentColor">
        <path d="M6.469 8.776 16.512 23h-4.464L2.005 8.776H6.47Zm-.004 7.9 2.233 3.164L6.467 23H2l4.465-6.324ZM22 2l-9.539 13.523-2.232-3.164L17.536 2H22ZM22 8.776V23h-3.659V13.97L22 8.776Z" />
      </svg>
    ),
  },
];
...
export function AskAiAboutUs({ className }: { className?: string } = {}) {
  const handleClick = (platform: Platform) => {
    ...
    if (navigator.clipboard?.writeText) {
      navigator.clipboard.writeText(HANA_PROMPT).catch(() => {});
    }
    // Open within the click gesture to avoid popup blocking.
    // Use a plain "_blank" target (no features string) so the ?q= query param is preserved.
